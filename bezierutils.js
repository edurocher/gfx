define([
	...
	bu.tAtLength = function (points, length) {
		...
		// length: Number
		//		The length.
		var t = 0, quadratic = points.length === 6, currentLen = 0, splitCount = 0, splitFunc = quadratic ?
			splitQBezierAtT : splitBezierAtT;
		var _compute = function (p, error) {
			...
			// chord length
			var chord = quadratic ? distance(points[0], points[1], points[4], points[5]) :
	...
	var splitQBezierAtT = function (points, t) {
		...
		// de Casteljau
		var r = 1 - t, r2 = r * r, t2 = t *
				t, p1x = points[0], p1y = points[1], cx = points[2], cy = points[3], p2x = points[4], p2y = points[5],

			ax = r * p1x + t * cx, ay = r * p1y + t * cy, bx = r * cx + t * p2x, by = r * cy + t * p2y, px = r2 * p1x +
				2 * r * t * cx + t2 * p2x, py = r2 * p1y + 2 * r * t * cy + t2 * p2y;
