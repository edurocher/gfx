define([
	...
	function toDashedCurveTo(/*Array||CanvasRenderingContext2D*/ctx, /*shape.Path*/shape, /*Number[]*/points, /*Object*/
		prevResidue
		) {
		// summary:
		//		Builds a set of bezier (cubic || quadratic)curveTo' canvas instructions that represents a dashed
		//		stroke of the specified bezier geometry.

		var pts = [shape.last.x, shape.last.y].concat(points), quadratic = points.length === 4, ctx2d = !(ctx instanceof
			Array), api = quadratic ? "quadraticCurveTo" : "bezierCurveTo", curves = [];
		var residue = canvas.splitToDashedBezier(pts, shape.canvasDash, curves, prevResidue);
	...
	return dcl([CanvasShape, PathBase], {
		...
		_moveToA: function (result, action, args, doDash) {
			...
			for (var i = 2; i < args.length; i += 2) {
				...
				if (doDash) {
					this._dashResidue =
						canvas.toDashedLineTo(doDash, this, args[i - 2], args[i - 1], args[i], args[i + 1],
							this._dashResidue);
				}
			}
		...
		_moveToR: function (result, action, args, doDash) {
			...
			for (var i = 2; i < args.length; i += 2) {
				...
				if (doDash) {
					this._dashResidue =
						canvas.toDashedLineTo(doDash, this, doDash[doDash.length - 1][0], doDash[doDash.length - 1][1],
							this.last.x, this.last.y, this._dashResidue);
				}
			}
			this.lastControl = {};
		},
		_lineToA: function (result, action, args, doDash) {
			for (var i = 0; i < args.length; i += 2) {
				if (doDash) {
					this._dashResidue =
						canvas.toDashedLineTo(doDash, this, this.last.x, this.last.y, args[i], args[i + 1],
							this._dashResidue);
				}
				result.push("lineTo", [args[i], args[i + 1]]);
			}
		...
		_hLineToA: function (result, action, args, doDash) {
			for (var i = 0; i < args.length; ++i) {
				...
				if (doDash) {
					this._dashResidue =
						canvas.toDashedLineTo(doDash, this, doDash[doDash.length - 1][0], doDash[doDash.length - 1][1],
							args[i], this.last.y, this._dashResidue);
				}
			}
		...
		_vLineToA: function (result, action, args, doDash) {
			for (var i = 0; i < args.length; ++i) {
				...
				if (doDash) {
					this._dashResidue =
						canvas.toDashedLineTo(doDash, this, doDash[doDash.length - 1][0], doDash[doDash.length - 1][1],
							this.last.x, args[i], this._dashResidue);
				}
			}
			this.last.y = args[args.length - 1];
			this.lastControl = {};
		...
		_curveToR: function (result, action, args, doDash) {
			for (var i = 0; i < args.length; i += 6) {
				result.push("bezierCurveTo", [
					this.last.x + args[i], this.last.y + args[i + 1], this.lastControl.x = this.last.x + args[i + 2],
					this.lastControl.y = this.last.y + args[i + 3], this.last.x + args[i + 4], this.last.y + args[i + 5]
				]);
				if (doDash) {
					this._dashResidue = toDashedCurveTo(doDash, this, result[result.length - 1], this._dashResidue);
				}
		...
		_smoothCurveToA: function (result, action, args, doDash) {
			for (var i = 0; i < args.length; i += 4) {
				...
				result.push("bezierCurveTo", [
					...
					valid ? 2 * this.last.y - this.lastControl.y : this.last.y, args[i], args[i + 1], args[i + 2],
					args[i + 3]
				]);
				if (doDash) {
					this._dashResidue = toDashedCurveTo(doDash, this, result[result.length - 1], this._dashResidue);
		...
		_smoothCurveToR: function (result, action, args, doDash) {
			for (var i = 0; i < args.length; i += 4) {
				...
				result.push("bezierCurveTo", [
					valid ? 2 * this.last.x - this.lastControl.x : this.last.x,
					valid ? 2 * this.last.y - this.lastControl.y : this.last.y, this.last.x + args[i],
					this.last.y + args[i + 1], this.last.x + args[i + 2], this.last.y + args[i + 3]
				]);
				if (doDash) {
					this._dashResidue = toDashedCurveTo(doDash, this, result[result.length - 1], this._dashResidue);
				}
		...
		_qCurveToR: function (result, action, args, doDash) {
			for (var i = 0; i < args.length; i += 4) {
				result.push("quadraticCurveTo", [
					this.lastControl.x = this.last.x + args[i], this.lastControl.y = this.last.y + args[i + 1],
					this.last.x + args[i + 2], this.last.y + args[i + 3]
				]);
				if (doDash) {
					this._dashResidue = toDashedCurveTo(doDash, this, result[result.length - 1], this._dashResidue);
				}
		...
		_qSmoothCurveToA: function (result, action, args, doDash) {
			for (var i = 0; i < args.length; i += 2) {
				...
				result.push("quadraticCurveTo", [
					...
					this.lastControl.y = valid ? 2 * this.last.y - this.lastControl.y : this.last.y, args[i],
					args[i + 1]
				]);
				if (doDash) {
					this._dashResidue = toDashedCurveTo(doDash, this, result[result.length - 1], this._dashResidue);
		...
		_qSmoothCurveToR: function (result, action, args, doDash) {
			for (var i = 0; i < args.length; i += 2) {
				...
				result.push("quadraticCurveTo", [
					...
					this.last.x + args[i], this.last.y + args[i + 1]
				]);
				if (doDash) {
					this._dashResidue = toDashedCurveTo(doDash, this, result[result.length - 1], this._dashResidue);
				}
		...
		_arcTo: function (result, action, args, doDash) {
			...
			for (var i = 0; i < args.length; i += 7) {
				...
				var arcs = ga.arcAsBezier(this.last, args[i], args[i + 1], args[i + 2], args[i + 3] ? 1 : 0,
					args[i + 4] ? 1 : 0, x1, y1);
		...
		_closePath: function (result, action, args, doDash) {
			...
			if (doDash) {
				this._dashResidue =
					canvas.toDashedLineTo(doDash, this, this.last.x, this.last.y, doDash[1][0], doDash[1][1],
						this._dashResidue);
			}
			this.lastControl = {};
		},
